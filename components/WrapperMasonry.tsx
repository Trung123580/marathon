      ...
      >
        {data.map((photo: any) => (
          <div key={photo.id} className='relative group' onContextMenu={onClickRightMouse}>
            ...
            {!!photo?.isPaid ? <span className='text-white w-full text-center showAnimation group-hover:block absolute bottom-0 left-1/2 -translate-x-1/2  bg-red-500 py-1 text-sm rounded-bl-lg rounded-br-lg'>{t?.event?.bought}</span>: <button className=' md:hidden text-white showAnimation group-hover:block w-full absolute bottom-0 left-1/2 -translate-x-1/2  bg-red-500 py-1 text-sm rounded-bl-lg rounded-br-lg'>{t?.search?.buy}</button>}
            {/* {!!photo?.isUser ? (
